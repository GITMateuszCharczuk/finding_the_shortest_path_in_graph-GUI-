...
public class MainController {
    ...
    public void launchGenerate(ActionEvent e) throws IOException {
        ...
        if (inpFileTextField.getText() != "") {
            ...
            GenerationGraphPane.generationGraphPane(generationPane, canvas, gc, row, col, g, e);
            g.printToScreen();
            wasBfsChecked = false;
    ...
    public void launchDijkstra(ActionEvent e) {
        ...
        int y = -2;
        try {
            x = colTextField.getText() != "" ? Integer.parseInt(colTextField.getText()) : -2;
            y = rowTextField.getText() != "" ? Integer.parseInt(rowTextField.getText()) : -2;
        } catch (NumberFormatException err) {
            stdErr.setText(stdErr.getText() + "Błąd w konwersji ciągu znaków na liczbę!!!\n");
            dialogue.setBackground(new Background(new BackgroundFill(Color.RED, null, null)));
            return;
        ...
        if (g != null) {
            if (wasBfsChecked) {
                if (isBfsGood) {
                    ...
                    DijkstraResultToGraphPane.dijkstraResultToGraphPane(generationPane, answer, canvas, gc, x, y);
    ...
    public void launchGraphSlicing(ActionEvent e) {
        ...
        int howManySlices = -2;
        try {
            howManySlices = !Objects.equals(sliceTextField.getText(), "") ? Integer.parseInt(sliceTextField.getText()) : -2;
        } catch (NumberFormatException err) {
            stdErr.setText(stdErr.getText() + "Błąd w konwersji ciągu znaków na liczbę!!!\n");
            dialogue.setBackground(new Background(new BackgroundFill(Color.RED, null, null)));
            return;
        ...
        if (g != null) {
            ...
            int isGood = GraphSlicing.graph_slice(g, howManySlices);
        } else {
            stdErr.setText(stdErr.getText() + "Graf nie został wygenerowany\n");
            dialogue.setBackground(new Background(new BackgroundFill(Color.RED, null, null)));
        }
    ...
    public void printToFile() {
        ...
        if (!Objects.equals(outFileTextField.getText(), "")) {
            try {
                if (g != null)
